...
function formatearAParaInput(fechaStr) {
    ...
    if (fechaStr.includes("-") && fechaStr.split("-")[2]?.length === 4) {
        ...
        return `${anio}-${mes}-${anio}`; // Corregido formato de fecha input
    }
    return fechaStr;
}
...
function buscarEnFirebase() {
    ...
    db.ref("rutas").once("value")
        .then((snapshot) => {
            resultsList.innerHTML = "";
            if (!snapshot.exists()) {
                resultsList.innerHTML = `<div class="text-center py-2 text-danger">No se encontraron registros en el nodo 'rutas'.</div>`;
                return;
            }
            ...
            snapshot.forEach((childSnapshot) => {
                ...
                if (registro) {
                    ...
                    // Si el input de búsqueda está vacío o coincide con cliente o número
            ...
            registros.forEach((item) => {
                ...
                a.innerHTML = `
                    <div>
                        <span class="badge bg-warning text-dark me-2">Ruta: ${item.numeroRecepcion || 'S/N°'}</span>
                        <strong>Cliente:</strong> ${item.cliente || 'Sin Cliente'} 
                        <small class="text-muted ms-2">(Fecha: ${item.fecha || 'N/A'})</small>
                    </div>
                    <span class="badge bg-primary px-3 py-2"><i class="fa-solid fa-arrow-pointer me-1"></i> Seleccionar y Cargar</span>
                `;
                a.addEventListener("click", (e) => {
                    e.preventDefault();
                    cargarRutaEnFormulario(item);
                });
...
function cargarRutaEnFormulario(data) {
    currentRecordKey = data.key;

    // Manejo seguro de fecha para el input type="date"
    let fechaVal = data.fecha || "";
    if (fechaVal.includes("-") && fechaVal.split("-")[0].length === 2) {
        const [dia, mes, anio] = fechaVal.split("-");
    ...
    alert(`¡Hoja de Ruta de "${data.cliente || 'Cliente'}" cargada correctamente!`);
}
...
function guardarEnFirebase() {
    ...
    const uniqueKey = clienteSanitizado + "_" + Date.now();
